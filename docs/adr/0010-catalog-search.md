...
Status: Accepted; local execution selected (see ADR 0011)
...
The user subsequently selected local-computer/server execution. ADR 0011 supersedes the pending external-provider decision for embeddings. Ordinary search remains first; local semantic fallback is used only when ordinary results are empty.
