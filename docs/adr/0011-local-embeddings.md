...
Live testing found that English SKU fragments could reverse near-tied Chinese candidates. Semantic text therefore includes only name, short name, model, specification and named attributes; identifiers remain in deterministic search. The model identity also includes document format catalog-v2, ensuring existing projections rebuild when the input representation changes.
