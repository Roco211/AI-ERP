...
import { ArrowLeft, ArrowUpRight, Bot, CalendarDays, CheckCheck, ChevronDown, ClipboardList, Copy, Database, History, MoreHorizontal, PackageSearch, RefreshCw, ShoppingBag, Sparkles, User } from "lucide-react";
...
import {
  ...
  type Conversation, type ConversationsPage, type Turn,
...
function AuthorizedAssistant({ permissions }: { permissions: string[] }) {
  ...
  const contextTurns = conversation?.turns.filter((turn) => (turn.evidence ?? []).length || turn.proposal) ?? [];
  const focused = contextTurns.find((turn) => turn.id === contextTurn) ?? contextTurns.at(-1);
  const pendingCount = conversation?.turns.filter((turn) => turn.proposal?.status === "PENDING").length ?? 0;
  ...
  function selectConversation(value: string | null) {
    setChosen(value); setContextTurn(null); setCopyStatus(null);
  ...
  return <section aria-label="AI 助手" className="assistant-experience flex h-full min-h-0 min-w-0 flex-col">
    ...
    {(submission.error || submission.notice || submission.busy || unavailable) && <section aria-label="操作反馈" className="max-h-40 shrink-0 space-y-2 overflow-y-auto border-b border-border/60 bg-background px-4 py-3 text-sm">
      <SubmissionFeedback value={submission} onRefresh={() => void refresh()} />
      {submission.busy && <p role="status" className="text-xs text-muted-foreground">正在提交并等待处理结果…</p>}
    ...
    <ChatApp open={false} openMobile={false} className="assistant-grid flex-1 rounded-none border-0" data-panel={panel}>
      ...
      <section aria-label="助手对话" data-assistant-panel="chat" className="assistant-chat min-h-0 min-w-0 flex-col bg-background">
        <header className="flex min-h-16 shrink-0 items-center gap-3 border-b border-border/60 px-4 py-3 sm:px-5">
          <span className="grid size-9 shrink-0 place-items-center rounded-xl border border-border bg-muted/50"><Sparkles className="size-4" aria-hidden /></span>
          <div className="min-w-0 flex-1"><h2 id="assistant-chat-heading" tabIndex={-1} className="truncate text-sm font-medium outline-none">{conversation?.title ?? "经营助手"}</h2>
            <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><span aria-hidden className={`size-1.5 rounded-full ${running || submission.busy ? "bg-amber-400" : status.data?.configured ? "bg-emerald-500" : "bg-muted-foreground/40"}`} />{running || submission.busy ? "正在处理你的请求" : "查询业务记录，开单先复核"}</p>
          </div>
        ...
        <MessageScroller key={id ?? "empty"} label="对话记录" busy={running || submission.busy}
          followOutput className="min-h-0 flex-1" viewportClassName="px-4 py-6 sm:px-6"
          ...
          {!conversation?.turns.length && (!id || !detail.isPending) && !unavailable && <div className="mx-auto flex min-h-full max-w-lg flex-col justify-center py-5">
            ...
            <p className="mt-3 text-sm leading-7 text-muted-foreground">从一条库存查询开始，也可以整理经营情况、准备一份待复核的订单。</p>
          ...
          <MessageGroup spacing="default">
            {conversation?.turns.map((turn) => <div key={turn.id} className="min-w-0 space-y-5 pb-5">
              ...
              <Message from="assistant" aria-label="经营助手回复"><MessageAvatar aria-hidden><Bot /></MessageAvatar>
                <MessageContent className="gap-3"><MessageHeader>经营助手</MessageHeader>
                  ...
                  {turn.answer && <MessageBubble variant="soft" animateIn={false}><MessageBubbleContent><p className="whitespace-pre-wrap break-words text-sm leading-7">{turn.answer}</p></MessageBubbleContent></MessageBubble>}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {!!(turn.evidence ?? []).length && <Button size="sm" variant="outline" onClick={() => openContext(turn)}><Database className="size-3.5" aria-hidden /><span>查看业务依据</span><span className="text-muted-foreground">{(turn.evidence ?? []).length}</span><ArrowUpRight className="size-3" aria-hidden /></Button>}
                    {turn.proposal && <Button size="sm" variant="outline" onClick={() => openContext(turn)}><ClipboardList className="size-3.5" aria-hidden />{turn.proposal.status === "PENDING" ? "复核草稿" : "查看草稿结果"}</Button>}
                    {turn.answer && <Button size="icon" variant="ghost" aria-label="复制回复" className="size-8" onClick={async () => {
        ...
        <section aria-label="发送消息" className="shrink-0 space-y-3 border-t border-border/50 bg-background px-3 pb-3 pt-2 sm:px-5 sm:pb-4">
          ...
          <PromptInput id="assistant-question" aria-label="你的问题" sendLabel="发送问题" maxLength={4000} value={question}
            ...
            minRows={2} maxRows={5} className="rounded-3xl border-border bg-muted/35 p-3 focus-within:bg-muted/50"
            ...
            placeholder="问库存、查经营，或描述一份订单…" onValueChange={setQuestion} onSubmit={ask} />
