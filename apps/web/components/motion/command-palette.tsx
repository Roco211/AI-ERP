...
export function CommandPalette({
  ...
  useEffect(() => {
    ...
    const background = Array.from(document.body.children).filter((node): node is HTMLElement => node instanceof HTMLElement && !node.hasAttribute("data-command-overlay"));
    const inertStates = background.map((node) => node.inert);
    background.forEach((node) => { node.inert = true; });
    root.style.overflow = "hidden";
    document.body.style.overflow = "hidden";
    const trapFocus = (event: FocusEvent) => {
      if (event.target instanceof Node && panelRef.current && !panelRef.current.contains(event.target)) inputRef.current?.focus();
    };
        ...
        <PresenceGate key="backdrop">
          {({ gate }) => (
            <motion.button
              ...
              aria-hidden={gate.inert}
              data-command-overlay="backdrop"
              tabIndex={-1}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{
