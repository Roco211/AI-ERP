...
export function Table<T>({
  ...
  const virtualItems = virtualized ? virtualizer.getVirtualItems() : sortedRows.map((_, index) => ({ index, start: 0, end: 0 }));
  ...
  return (
    ...
    >
      <div
        ...
        style={{ height: virtualized ? height : undefined }}
      >
        <table
                    ...
                    >
                      ...
                      {orderedColumns.map((column) => (
                        <td
                          key={column.key}
                          className={cn(
                            virtualized ? "truncate px-4 text-foreground" : "px-4 py-3 align-top whitespace-normal break-words text-foreground",
