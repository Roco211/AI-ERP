...
> {
  ...
  /** ERP native form/RHF bridge; receives the real event, never a fabricated target. */
  onNativeChange?: ChangeEventHandler<HTMLInputElement>;
  /** Preserve uncontrolled DOM values written by native forms/RHF setValue/reset. */
  nativeForm?: boolean;
...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  const handleChange = (next: string) => {
    if (!controlled && !nativeForm) setInternal(next);
    onChange?.(next);
  };
    ...
    >
      ...
      >
        ...
        <input
          ...
          id={id}
          type={type}
          value={type === "file" || (nativeForm && !controlled) ? undefined : value}
          defaultValue={nativeForm && !controlled && type !== "file" ? defaultValue : undefined}
          disabled={disabled}
