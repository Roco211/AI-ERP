...
function focusableIn(panel: HTMLElement | null) {
  return panel ? Array.from(panel.querySelectorAll<HTMLElement>(FOCUSABLE_SELECTOR)).filter((element) =>
    element.tabIndex >= 0 && !element.closest("[inert]") && element.getClientRects().length > 0 && getComputedStyle(element).visibility !== "hidden",
  ) : [];
}
  ...
  useEffect(() => {
    ...
    const focusFrame = requestAnimationFrame(() => {
      const firstFocusable =
        focusableIn(panelRef.current)[0];
...
export type AnimatedSidebarInsetProps = HTMLMotionProps<"div">;
...
export interface AnimatedSidebarMenuButtonProps {
  ...
  rel?: string;
  onSelect?: () => void;
  onNavigate?: (event: React.MouseEvent<HTMLAnchorElement | HTMLButtonElement>) => void;
  className?: string;
}
