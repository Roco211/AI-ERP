...
test("beUI prompt preserves multiline and IME input and blocks unconfigured or pending submissions", async ({ page, beuiIdentity }) => {
  ...
  try {
    ...
    configured = true;
    await page.getByLabel("对话操作", { exact: true }).click();
    ...
    await page.keyboard.press("Control+k");
    const palette = page.getByRole("dialog", { name: "快捷导航", exact: true });
    const search = palette.getByRole("combobox", { name: "搜索页面", exact: true });
    await search.fill("库存");
    await search.press("ArrowDown");
    const activePage = await search.getAttribute("aria-activedescendant");
    expect(activePage).toBeTruthy();
    await expect(page.locator(`[id="${activePage}"]`)).toContainText("库存");
